...
public class EventDAO
{
    ...
    {
        SqlConnection con = new SqlConnection(connectionString);
        SqlCommand cmd = new SqlCommand("UPDATE Event SET event_start=@date, event_end=@location, all_day=@time WHERE event_id=@event_id", con);
        cmd.Parameters.Add("@name", SqlDbType.DateTime).Value = start;
        cmd.Parameters.Add("@location", SqlDbType.DateTime).Value = end;
        cmd.Parameters.Add("@date", SqlDbType.Int).Value = id;
        cmd.Parameters.Add("@time", SqlDbType.Bit).Value = allDay;
    ...
    public static int addEvent(CalendarEvent cevent)
    {
        ...
        //insert
        SqlConnection con = new SqlConnection(connectionString);
        SqlCommand cmd = new SqlCommand("INSERT INTO Event(title, description, event_start, event_end, all_day) VALUES(@name, @description, @date, @location, @time)", con);
        ...
        cmd.Parameters.Add("@date", SqlDbType.DateTime).Value = cevent.date;
        cmd.Parameters.Add("@location", SqlDbType.DateTime).Value = cevent.location;
        cmd.Parameters.Add("@time", SqlDbType.Bit).Value = cevent.time;
        cmd.Parameters.Add("@requirement", SqlDbType.Bit).Value = cevent.requirement;
        ...
        using (con)
        {
            ...
            cmd = new SqlCommand("SELECT max(event_id) FROM Event where title=@name AND description=@description AND event_start=@date AND event_end=@location AND all_day=@time", con);
            cmd.Parameters.Add("@name", SqlDbType.VarChar).Value = cevent.name;
            cmd.Parameters.Add("@description", SqlDbType.VarChar).Value = cevent.description;
            cmd.Parameters.Add("@date", SqlDbType.DateTime).Value = cevent.date;
            cmd.Parameters.Add("@location", SqlDbType.DateTime).Value = cevent.location;
